...
export default async function RevenueChart(){ // hace el componente asincrono para poder hacer la peticion a la api
  ...
  const { yAxisLabels, topLabel } = generateYAxis(revenue); // genera las etiquetas del eje y y el valor mas alto de la grafica
    ...
    <div className="w-full md:col-span-4">
      ...
      <div className="rounded-xl bg-gray-50 p-4">
        <div className="sm:grid-cols-13 mt-0 grid grid-cols-12 items-end gap-2 rounded-md bg-white p-4 md:gap-4">
          ...
          >
            {yAxisLabels.map((label) => ( // mapea las etiquetas del eje y
              <p key={label}>{label}</p>
            ))}
          </div>

          {revenue.map((month) => ( // mapea los meses
